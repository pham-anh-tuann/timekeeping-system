...
<head>
    <meta charset="UTF-8">
    <title><?= $title ?? 'Sửa Nhân viên' ?></title>
    ...
    <style>
        ...
        .btn-submit { background: var(--primary); color: #0f172a; border: none; padding: 14px 25px; border-radius: 10px; font-weight: 800; cursor: pointer; transition: 0.3s; width: 100%; font-size: 15px; box-shadow: 0 4px 15px rgba(46, 255, 232, 0.2); margin-top: 10px;}
        .btn-submit:hover { background: #5cffe0; transform: translateY(-2px); box-shadow: 0 8px 20px rgba(46, 255, 232, 0.4); }
...
<div class="main">
    ...
    <div class="content">
        ...
        <div class="card">
            <h2 class="page-title" style="margin-bottom: 25px;"><i class="fa-solid fa-pen-to-square" style="color: var(--primary-dark);"></i> Cập nhật: <?= htmlspecialchars($editUser['full_name']) ?></h2>
            <form action="/timekeeping-system/public/admin/employees/edit" method="POST">
                <input type="hidden" name="id" value="<?= $editUser['id'] ?>">
                
                <div class="form-group">
                    <label>Mã Nhân Viên (Worker ID)</label>
                    <input type="text" name="worker_id" class="form-control" required value="<?= htmlspecialchars($editUser['worker_id']) ?>">
                </div>
                <div class="form-group">
                    <label>Họ và Tên</label>
                    <input type="text" name="full_name" class="form-control" required value="<?= htmlspecialchars($editUser['full_name']) ?>">
                </div>
                
                <div class="form-group">
                    <label>Vai trò / Chức vụ</label>
                    <select name="role" class="form-control" required>
                        <option value="Employee" <?= ($editUser['role'] === 'Employee') ? 'selected' : '' ?>>Nhân viên (Employee)</option>
                        <option value="Manager" <?= ($editUser['role'] === 'Manager') ? 'selected' : '' ?>>Trưởng phòng (Manager)</option>
                ...
                <div class="form-group">
                    ...
                    <select name="shift_id" class="form-control" required>
                        <option value="" disabled <?= empty($editUser['shift_id']) ? 'selected' : '' ?>>-- Chọn ca làm việc --</option>
                        <?php foreach($shifts as $s): ?>
                            <option value="<?= $s['id'] ?>" <?= ($editUser['shift_id'] == $s['id']) ? 'selected' : '' ?>><?= htmlspecialchars($s['shift_name']) ?> (<?= substr($s['start_time'],0,5) ?> - <?= substr($s['end_time'],0,5) ?>)</option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="daily_salary" class="form-control" required value="<?= $editUser['daily_salary'] ?>">
                </div>
