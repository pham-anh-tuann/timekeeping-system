...
<head>
    <meta charset="UTF-8">
    <title><?= $title ?? 'Khiếu nại chấm công' ?></title>
    ...
    <style>
        ...
        .btn-action { padding: 8px 12px; border-radius: 8px; font-size: 12px; font-weight: 700; text-decoration: none; color: white; display: inline-flex; align-items: center; gap: 6px; transition: 0.3s; border: none; cursor: pointer; margin-right: 5px; margin-bottom: 5px; box-shadow: 0 4px 10px rgba(0,0,0,0.1); }
        .btn-approve { background: #10b981; } .btn-approve:hover { background: #059669; transform: translateY(-2px); box-shadow: 0 6px 15px rgba(16, 185, 129, 0.3); }
        .btn-reject { background: #ef4444; } .btn-reject:hover { background: #dc2626; transform: translateY(-2px); box-shadow: 0 6px 15px rgba(239,68,68,0.3); }
        
        .time-box { display: inline-block; font-size: 13px; font-weight: 700; margin-bottom: 4px; padding: 4px 8px; background: #f8fafc; border-radius: 6px; border: 1px solid #e2e8f0; }
...
<?php 
    ...
    // Tính toán Thống kê Khiếu Nại
    $pendingComplaintsCount = 0; $approvedComplaints = 0; $rejectedComplaints = 0;
    foreach($complaints ?? [] as $c) {
        if($c['status'] === 'Pending') $pendingComplaintsCount++;
        elseif($c['status'] === 'Approved') $approvedComplaints++;
        else $rejectedComplaints++;
    }
...
<div class="sidebar">
    ...
    <a href="/timekeeping-system/public/admin/complaints" class="nav-item active"><i class="fa-solid fa-circle-exclamation"></i> Khiếu nại</a>
...
<div class="main">
    ...
    <div class="content">
        <h2 class="page-title">Xử lý khiếu nại chấm công</h2>
        
        <div class="summary-cards">
            <div class="stat-box"><div class="info"><span class="title">Đơn Chờ Xử Lý</span><span class="value" id="count-pending"><?= $pendingComplaintsCount ?></span></div><div class="icon icon-yellow"><i class="fa-solid fa-file-circle-exclamation"></i></div></div>
            <div class="stat-box"><div class="info"><span class="title">Đã Duyệt (Sửa giờ)</span><span class="value" id="count-approved"><?= $approvedComplaints ?></span></div><div class="icon icon-green"><i class="fa-solid fa-wrench"></i></div></div>
            <div class="stat-box"><div class="info"><span class="title">Bị Từ Chối</span><span class="value" id="count-rejected"><?= $rejectedComplaints ?></span></div><div class="icon icon-red"><i class="fa-solid fa-ban"></i></div></div>
        </div>

        <div class="filter-bar">
            <div class="filter-group" style="flex: 2;">
                <i class="fa-solid fa-magnifying-glass"></i>
                <input type="text" id="cSearch" class="filter-input" placeholder="Tìm theo Tên hoặc Mã NV..." onkeyup="filterComplaints()">
            </div>
            <div class="filter-group">
                <select id="cStatus" class="filter-select" onchange="filterComplaints()">
                    <option value="all">Tất cả tình trạng đơn</option>
                    <option value="pending">Đang chờ xử lý</option>
                    <option value="approved">Đã giải quyết</option>
        ...
        <div class="card">
            <table id="complaintTable">
                <thead>
                    <tr><th>Nhân viên</th><th>Ngày công</th><th>Hệ thống ghi nhận</th><th>Đề xuất sửa thành</th><th>Lý do</th><th>Trạng thái</th><th>Thao tác</th></tr>
                </thead>
                <tbody>
                    <?php foreach($complaints ?? [] as $c): ?>
                    <tr class="complaint-row" data-status="<?= strtolower($c['status']) ?>">
                        <td>
                            <strong class="col-name" style="color: #0f172a; font-size: 14px;"><?= htmlspecialchars($c['full_name']) ?></strong><br>
                            <span class="col-id" style="font-size:12px; color:var(--text-muted); font-weight:600;"><?= htmlspecialchars($c['worker_id']) ?></span>
                        </td>
                        <td style="font-weight: 700; color: #334155;"><?= date('d/m/Y', strtotime($c['work_date'])) ?></td>
                        <td>
                            <div class="time-box">Vào: <span style="color:var(--danger);"><?= htmlspecialchars($c['old_in'] ?? '--:--') ?></span></div><br>
                            <div class="time-box">Ra: <span style="color:var(--danger);"><?= htmlspecialchars($c['old_out'] ?? '--:--') ?></span></div>
                        </td>
                        <td>
                            <div class="time-box">Vào: <span style="color:var(--success);"><?= htmlspecialchars($c['suggested_time']) ?></span></div><br>
                            <div class="time-box">Ra: <span style="color:var(--success);"><?= htmlspecialchars($c['suggested_out_time'] ?? '--:--') ?></span></div>
                        </td>
                        <td style="max-width: 200px; font-style: italic; color: #475569;">"<?= htmlspecialchars($c['reason']) ?>"</td>
                        <td><span class="badge <?= $c['status'] ?>"><?= $c['status'] ?></span></td>
                        <td style="min-width: 140px;">
                            <?php if($c['status'] === 'Pending'): ?>
                                <a href="/timekeeping-system/public/admin/complaints/approve?id=<?= $c['id'] ?>&status=Approved" class="btn-action btn-approve" onclick="return confirm('Duyệt và TỰ ĐỘNG SỬA GIỜ CÔNG?');"><i class="fa-solid fa-check"></i> Duyệt & Sửa</a><br>
                                <a href="/timekeeping-system/public/admin/complaints/approve?id=<?= $c['id'] ?>&status=Rejected" class="btn-action btn-reject"><i class="fa-solid fa-xmark"></i> Từ chối</a>
                            <?php else: ?>
                    ...
                    <?php endforeach; ?>
                    <tr id="noDataRow" style="display:none;"><td colspan="7" style="text-align:center; padding: 30px; color: #94a3b8;">Không tìm thấy khiếu nại nào!</td></tr>
                </tbody>
            </table>
        </div>
    </div>
...
function filterComplaints() {
    ...
    let rows = document.getElementsByClassName("complaint-row");
    let visibleCount = 0;
